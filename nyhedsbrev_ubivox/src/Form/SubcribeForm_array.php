...
/**
 * Provides a subscribe form using array-based subscription for Ubivox.
 *
...
class SubcribeForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['abonnementer']['ubivox_lister']['landsdaekkende_nyheder_fra_anklagemyndigheden'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('National news from the Prosecution Service'),
    ];
    ...
    '#type' => 'checkbox',
    '#title' => $this
    ->t('News from the State Prosecutor for Serious Economic
    ...
    $form['abonnementer']['ubivox_lister']['aarsrapporter_bekendtgoerelser_og_publikationer'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Annual reports, executive orders and publications'),
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    // Ikke tilmeldt nogen liste.
    if ($landsdaekkende_nyheder_fra_anklagemyndigheden == 0 && $aarsrapporter_bekendtgoerelser_og_publikationer == 0) {
      $this->messenger()->addError($this->t('Please Select at Least One List to Subscribe to.'));
      return;
    }
    ...
    if ($host == "anklagemyndigheden.dk") {
      ...
      if ($landsdaekkende_nyheder_fra_anklagemyndigheden == 1) {
        $list_array[] = $list_id_landsdaekkende_nyheder;
      }

      /*
      if ($nyheder_fra_soeik == 1) {
      $list_array[] = $list_id_nyheder_soeik;
      }
       */

      if ($aarsrapporter_bekendtgoerelser_og_publikationer == 1) {
        $list_array[] = $list_id_aarsrapporter_bekendtgoerelser_publikationer;
      }
      try {
        // Using opt-in to list IDs for user@example.com.
        ...
        if ($e->getCode() == 1003) {
          // Already subscribed - handled at end.
        }
      }
      // If ($landsdaekkende_nyheder_fra_anklagemyndigheden == 1
      // || $nyheder_fra_soeik == 1
      // || $aarsrapporter_bekendtgoerelser_og_publikationer == 1) {.
      if ($landsdaekkende_nyheder_fra_anklagemyndigheden == 1 || $aarsrapporter_bekendtgoerelser_og_publikationer == 1) {
        try {
          ...
          // Use $e->getCode() or $e->getMessage() for error handling.
        }
      }
    }
    else {
      ...
      if ($landsdaekkende_nyheder_fra_anklagemyndigheden == 1) {
        $list_TEST_array[] = $list_id_TEST_landsdaekkende_nyheder;
      }

      /*
      if ($nyheder_fra_soeik == 1) {
      $list_TEST_array[] = $list_id_TEST_nyheder_soeik;
      }
       */

      if ($aarsrapporter_bekendtgoerelser_og_publikationer == 1) {
        $list_TEST_array[] = $list_id_TEST_aarsrapporter_bekendtgoerelser_publikationer;
      }
    ...
    $this->messenger()->addStatus($email . ' ' . $this->t('You have received an email with a link that you must click to confirm the subscription.'));
  }
